    ...
    <UIContext.Provider value={{ isLoading, setIsLoading, toast, setToast, showToast }}>
      ...
      {toast && (
        <div className="fixed bottom-5 right-5 z-50 animate-bounce-short">
          <div className={`flex items-center gap-3 px-4 py-3 rounded-lg shadow-xl border text-sm font-medium transition-all ${
            toast.type === 'success' 
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] z-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-2xl shadow-2xl flex flex-col items-center gap-4 max-w-[200px] border border-slate-100 animate-fade-in">
            <div className="relative">
              <div className="w-12 h-12 rounded-full border-4 border-purple-100 border-t-purple-600 animate-spin"></div>
            </div>
            <p className="text-sm font-semibold text-slate-700 text-center animate-pulse">Processing...</p>
          </div>
        </div>
      )}
    </UIContext.Provider>
